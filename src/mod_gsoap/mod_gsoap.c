 ...
 * updated by Grigory Panov < gr1313 at mail dot ru >
 *
...
static const char *SoapSharedLibraries_getEntryPoints(SoapSharedLibraries *This, SoapSharedLibrary *pLib, pool *pTempPool, request_rec *r) {
 ...
 */
static const char *SoapSharedLibraries_loadAllLibraries(SoapSharedLibraries *This, pool *pTempPool, request_rec *r) {
...
do
	{
		...
		for (i = 0; i < This->m_pLibraries->nelts; i++) 
		{
			...
			if (NULL == pLib->m_hLibrary) 
			{
				...
				{
					void *pfun = (void*)DLSYM(pLib->m_hLibrary, APACHE_HTTPSERVER_ENTRY_POINT); // try find entry 
					if(NULL == pfun)
					{
						pszError = apr_psprintf(NULL == pTempPool ? This->m_pPool : pTempPool,"xana! `%d",This->m_pLibraries->nelts);
						return pszError;
					}
					else
					{
...
static int gsoap_handler(request_rec *r)
{
    ...
    /*char *pszResponse = apr_pcalloc(r->pool, nResponseBufferLen);*/
    /*gsoapConfiguration *pConfig = getConfiguration(r);*/
    /*gsoapRequestConfiguration *pRqConf = NULL;*/
    assert(NULL != pConfig);
    ...
    if (NULL != pIntf->fsoap_init) {
		...
		if (NULL != pIntf->fsoap_serve) {
			int nRet = (*pIntf->fsoap_serve)(psoap, r);
		} else {
